...
class MS_SL2_split_model(nn.Module):
    def __init__(self,
        ...
        self.gate_convs = nn.ModuleList(
            [nn.Conv1d(Sc, Sc, 1) for _ in range(Slice)]
        )
    ...
    def _cyclic_weight(self, channels, device):
        weight_index = th.arange(channels, device=device) % self.wList.numel()
        return self.wList[weight_index].view(1, channels, 1)
    ...
    def forward(self, x):
        ...
        for Slice in range(self.slice):
            ...
            cyclic_weight = self._cyclic_weight(
                skip_connection.size(1), skip_connection.device)
            temporal_gate = th.sigmoid(self.gate_convs[Slice](skip_connection))
            Slice_row_sum = skip_connection * cyclic_weight * temporal_gate
            if Slice == 0:
        ...
        # spks x n x S
        return [self.decoder_1d(x, squeeze=False).squeeze(1) for x in s]
